"""Fixtures shared by the resource-server tests.

Two kinds live here. `rsa_keypair` and `sign_token` build tokens this suite
signs itself, so the bearer middleware and the token handling around a live app
can be tested without Keycloak. `gitea` and `mint_obo` reach the running compose
stack and skip when it is absent, so `make test` is green on a clean checkout
and still exercises them when the stack is up.
"""
