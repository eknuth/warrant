...
Two kinds live here. `chain`, `make_request`, `policy_dir`, and `decision_log`
build the W5 authorization objects. `rsa_keypair`, `sign_token`, `gitea`, and
`mint_obo` build tokens this suite signs itself and reach the running compose
stack, skipping when it is absent, so `make test` is green on a clean checkout
and still exercises the live tests when the stack is up.
"""
