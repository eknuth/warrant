"""Fixtures for the authorization objects under `warrant/`.

`chain`, `make_source`, `make_request`, `policy_dir`, and `decision_log` build
the W5 types by hand, so a test over the engine or the decision log needs no
running stack and no model. They live here rather than in `tests/conftest.py`
so that the W5 fixtures sit together and the file that pytest loads stays a
list of the modules that carry fixtures.
"""
