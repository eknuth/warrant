...
per-task coroutine with `asyncio.gather`, and each coroutine is a whole run:
its own login, its own on-behalf-of exchange keyed to its own task id, its own
run directory, and its own MCP session.
...
async def run_concurrent(
    ...
    *,
    ...
    `asyncio.gather` with no `return_exceptions`, so one task's failure cancels
    the answer rather than being smuggled into the list as a value. A caller
    that wants partial outcomes catches the error at its own level.
    """
    if not tasks:
        return []
